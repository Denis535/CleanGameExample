...
- Program - это главный класс, отвечающий за инициализацию, обработку событий и разрешение зависимостей.
- DebugScreen - это класс отладочного экрана, показывающий некоторую полезную информацию.
- ProjectToolbar - это класс инструментов, которые вы можете использовать на панели инструментов.
- ProjectWindow - это класс продвинутого окна проекта, которое подсвечивает специальные папки и файлы в соответствующие им цвета.
...
Теперь давайте рассмотрим исходный код:
- UITheme - это класс аудио темы приложения.
  Заметьте, что он использует паттерн состояние, что позволяет вам легко переключаться между разными плейлистами.
- MainPlayList и GamePlayList - это классы состояний темы.
  Они проигрывают разные музыкальные плейлисты.
  А в некоторых случаях добавляют некоторые аудио эффекты.

- UIScreen - это класс графического пользовательского интерфейса приложения.
  Заметьте, что он использует state-driven архитектуру, что значит, что пользовательский интерфейс состоит из дерева состояний и дерева визуальных элементов.
...
- RootWidget - это класс корневого виджета.
  Он добавляет другие виджеты в дерево визуальных элементов.

- UIRouter - это класс менеджера приложения.
...
- SettingsWidget - это виджет настроек.
  Точнее это лишь родительский виджет для других виджетов, которые содержат сами настройки.
...
Теперь давайте рассмотрим исходный код:
- Application - это сущность, содержащая другие сущности и сервисы приложения.
- Storage, ProfileSettings, VideoSettings, AudioSettings - это классы, которые содержат значения, полученные из аргументов командной строки или загруженные из хранилища.
...
- Camera - это сущность камеры, которая снимает персонажа от третьего лица.
  Заметьте, что камера работает в разных режимах для живого и мертвого персонажа.

- CharacterBase - это базовый класс для всех персонажей, которые могут перемещаться, брать оружие, атаковать и быть атакованным.
  Заметьте, что вся низкоуровневая логика находится в классе-фасаде.
- PlayableCharacterBase - это базовый класс для всех играбельных персонажей.
- NonPlayableCharacterBase - это базовый класс для всех неиграбельных персонажей.
- PlayerCharacter - это сущность персонажа игрока, которая управляется игроком.
- EnemyCharacter - это сущность вражеского персонажа, которая управляется искусственным интеллектом и пытается убить персонажа игрока.

- WeaponBase - это базовый класс для всех оружий, которые могут стрелять.
- Gun - это сущность пистолета, которая может стрелять и наносить урон другим персонажем.
- Bullet - это сущность пули, которая при столкновении с персонажем наносит ему урон.

- World - это сущность мира, которая предоставляет вам все объекты мира.

Теперь давайте рассмотрим ассеты:
- Camera - префаб камеры
- PlayerCharacter - префаб игрового персонажа
- EnemyCharacter - префаб вражеского персонажа
...
Теперь давайте рассмотрим исходный код.
- Lock - это класс, запрещающий многочисленный доступ в одну область кода.
- Utils - это класс, содержащий разные полезные методы.
- GameObjectExtensions - это класс, содержащий расширения для GameObject.
- VisualElementExtensions - это класс, содержащий расширения для VisualElement.
- VisualElementFactory - это класс, содержащий методы для создания разных визуальных элементов.
...
- L - это класс со список всех меток ассетов.
- IDamageable - это интерфейс для нанесения урона сущности.
- ActorBase - это базовый класс для всех актеров, действующих под управлением игрока или искусственного интеллекта.
- MovableBody - это компонент реализующий простейшую физику тела персонажа.
- ThingBase - это базовый класс для всех вещей, которыми актеры могут владеть.
...
- UIThemeBase - это класс аудио темы приложения.
  Заметьте, что он использует паттерн состояние, что позволяет вам легко переключаться между разными состояниями.
- UIPlayListBase - это класс состояния темы.
  Он просто проигрывает текущий музыкальный плейлист.

- UIScreenBase - это класс графического пользовательского интерфейса приложения.
  Заметьте, что он использует state-driven архитектуру, что значит, что пользовательский интерфейс состоит из дерева состояний и дерева визуальных элементов.
  Это позволяет вам создавать пользовательский интерфейс из множества разных состояний и легко переключаться между ними.
  Я рассказывал об этой архитектуре в прошлых видео.
- UIWidgetBase - это класс состояния пользовательского интерфейса.
...
- UIViewBase - это класс визуального представления виджета.
  ...
  В некоторых случаях вьюшка может быть контейнером и содержать другие вьюшки.
  Обычно вьюшка корневого виджета размещает на экране все другие вьюшки.
  Хотя могут быть и более сложные случаи.

- UIRouterBase - это класс менеджера состояний приложения.
...
- StorageBase - это класс, содержащий разные значения, обычно загруженные из хранилища или полученные из аргументов окружения.
