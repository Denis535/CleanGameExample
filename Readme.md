...
- Декомпозиция
  ...
  Наш проект состоит из нескольких основных модулей и одного дополнительного модуля инфраструктуры.
- Разделение проекта и контента
  ...
  Но я использовал немного другой подход.
  Наш проект состоит из папок Project и Project.Content.
  Первая папка содержит только *.asmdef файлы, то есть только определения всех основных модулей.
  А вторая папка содержит все исходники и ассеты.
  Получается, что все исходники и ассеты находятся рядом друг с другом.
  Но в тоже время все исходники и ассеты хранятся в своих неймспейс-папках и не смешиваются.
  К счастью, с помощью *.asmref файлов, мы легко можем привязать любые папки с исходниками к конкретным модулям.

В итоге наш проект получился достаточно простым и удобным.
