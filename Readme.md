...
# 2. Организация удобной структуры проекта
========================================
...
А ориентироваться в таких проектах становится очень сложно.
...
- Первая идея — это декомпозиция проекта.
  Это идея, что сложный проект разделяется на более простые модули.
  А модули, в свою очередь, разделяется на еще более простые неймспейсы и классы.
  Нужно понимать, что проект можно декомпозировать по назначению кода или по уровню кода.
  ...
  В нашем проекте я использовал оба варианта.
  То есть наш проект содержит несколько основных модулей и один дополнительный модуль инфраструктуры.
- Вторая идея — это разделение проекта и контента.
  Это идея, что проект содержит только модули и исходники, а все ассеты хранятся отдельно.
  В более радикальном варианте, проект содержит только модули, а все исходники и ассеты хранятся где-то отдельно.
  В нашем проекте я использовал второй вариант.
  То есть наш проект содержит папки Project и Project.Content.
  Первая папка содержит только определения всех основных модулей.
  ...
  Обратите внимание, что в папке Plugins я храню библиотеки, которые не имеют прямого отношения к нашему проекту.

**Вывод**
Благодаря этим идеям, проекты получаются более удобными.
Все основные исходники и ассеты всегда будут у вас под рукой, а все второстепенное спрятано подальше.
...
# 3. Организация качественной архитектуры проекта
========================================
В этом видео мы подробно рассмотрим принципы организации качественной архитектуры проекта.

**Проблема**
При разработке достаточно сложных проектов, проекты декомпозируется на множество разных модулей, неймспейсов и классов.
В идеале каждая единица проекта должна иметь единую и понятную ответственность и минимальное количество зависимостей.
Но в реальности, проекты легко превращается в кучу файлов с непонятными ответственностями и запутанными зависимостями.
А разобраться в таких проектах становится очень сложно.
Поэтому нужны парадигмы, которые четко и ясно определят правила разработки проектов.

**Решение**
Для более качественной организации архитектуры проекта можно использовать следующие идеи:
- Первая идея — это Чистая Архитектура.
  Это идея, что проект состоит из разных модулей, причем каждый модуль должен иметь свою конкретную и единую ответственность.
  То есть проект может состоять из точки входа, презентации, приложения, предметной области и модуля инфраструктуры.
  Или проект может состоять из высокоуровневых и низкоуровневых модулей.
  Или же просто из более важных и менее важных модулей.

  В итоге получается, что каждая ответственность отделена от других ответственностей.
  ...
  В итоге получается, что все самое важное в проекте очищено от разного мусора.
  А главный модуль становится максимально простым и чистым.

- Вторая идея — это Domain-Driven Дизайн.
  Это идея, что модули предметной области являются самыми главными в проекте.
  Заметьте, что предметная область — это множество сущностей, выполняющих бизнес задачи приложения и отражающие объекты реального или придуманного мира.
  ...
  Причем в идеале нужно, чтобы предметная область не имела прямых зависимостей на технические модули.
  В этом случае можно использовать инверсию зависимостей.
  То есть предметная область не имеет зависимостей на свою инфраструктуру, вместо этого инфраструктура имеет зависимости на предметную область.
  Таким образом предметная область может предоставлять некоторые абстракции, а инфраструктура может реализовать эти абстракции.
  Следовательно, предметная область может использовать сложную инфраструктуру через свои простые абстракции и при этом вообще не иметь прямых зависимостей на инфраструктуру.
  В итоге получается, что предметная область полностью очищена от технических деталей.
  А ее разработка полностью независима от всего остального.
- Третья идея — это View-Driven UI.
...
- Четвертая идея — это State-Driven UI.
...
**Вывод**
Благодаря этим идеям, проекты получаются достаточно логичными и однообразными.
Все модули, неймспейсы и классы всегда будут иметь единую и понятную ответственность и выполнять свои задачи.
