...
**Проблема**
При разработке достаточно сложных проектов, проект декомпозируется на множество модулей, неймспейсов и классов.
В идеале каждая единица проекта должна иметь понятную и единую ответственность, и минимальный список зависимостей.
Но в реальности, к сожалению, достичь это очень тяжело.
Все становится еще сложнее, когда над проектом работает много разработчиков с разными привычками и опытом.
Проект превращается в огромное количество файлов с непонятными ответственностями и запутанными зависимостями.
