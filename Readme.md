...
Наш проект включает в себя несколько основных модулей.
И дополнительный модуль инфраструктуры.

Все основные модули находятся в папках Project и Project.Assemblies.
А модуль инфраструктуры является частью одноименного пакета.

Но обратите внимание, что все исходники и ассеты находятся в папке Project.
А папка Project.Assemblies содержит только asmdef файлы.

Давайте рассмотрим подробнее наш проект:
- Красная папка Project содержит зеленные и желтые папки.
- Зеленные папки содержат исходники, а имена папок отражают пространства имен этих исходников.
- Желтые папки содержат ассеты, а имена папок отражают адреса этих ассетов.
Заметьте, что все папки с исходниками так же содержат asmref файлы, которые определяют к какому модулю относятся эти исходники.

В итоге наш проект получился простым и удобным.
