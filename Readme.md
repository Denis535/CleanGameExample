...
- DebugScreen - это отладочный экран, показывающий некоторую полезную информацию.
...
  Заметьте, что я использовал паттерн состояние, что позволяет вам легко переключаться между разными плейлистами.
...
- Storage, ProfileSettings, VideoSettings, AudioSettings - это объекты, которые содержат значения, загруженные из хранилища или полученные из аргументов командной строки.
...
- CharacterBase - это сущность персонажа, которая может перемещаться, использовать оружие, атаковать и быть атакованой.
...
- WeaponBase - это сущность оружия, которая может стрелять.
