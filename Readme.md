...
- Вторая идея — это разделение проекта и контента.
  ...
  При необходимости, мы можем даже привязать несколько папок с исходниками к одному модулю.
  И таким образом уменьшить вложенность папок.
  Конечно же некоторые ассеты все же лучше хранить где-то отдельно.
  Например, в папке Plugins я храню ассеты, которые не имеют прямого отношения к нашему проекту.
