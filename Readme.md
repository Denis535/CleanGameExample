...
- ProjectWindow - это класс более продвинутого окна проекта, которое подсвечивает специальные папки и файлы в соответствующие им цвета.
...
Теперь давайте рассмотрим исходный код:
- UITheme - это класс аудио темы нашего приложения.
  Заметьте, что этот класс использует паттерн состояние, что позволяет вам легко переключаться между разными плейлистами.
- PlayList, MainPlayList и GamePlayList - это классы музыкальных плейлистов.

- UIScreen - это класс графического пользовательского интерфейса нашего приложения.
  Заметьте, что этот класс использует state-driven архитектуру, что позволяет вам создавать пользовательский интерфейс из множества разных виджетов и легко переключаться между ними.
  Это значит, что пользовательский интерфейс состоит из дерева виджетов и дерева визуальных элементов.
...
- UIRouter - это класс менеджера нашего приложения.
...
Теперь давайте рассмотрим исходный код:
- Application - это сущность, содержащая другие сущности и сервисы нашего приложения.
...
- IDamageable - это интерфейс для нанесения уронов сущностям.
...
- MovableBody - это компонент реализующий простейшую физику тела персонажа.
  Заметьте, что тело может быть в физическом или кинематическом состоянии.
- ThingBase - это базовый класс для всех вещей, которыми актеры могут владеть.
  Заметьте, что вещи могут быть в физическом или кинематическом состоянии.
