...
- ProjectWindow - это более удобное окно проекта, которое подсвечивает специальные папки и файлы в соответствующие цвета.
...
- UIRouter - это менеджер нашего приложения.
  Этот менеджер предоставляет методы для управления нашим приложением, такие как загрузить, перезагрузить и выгрузить игру, а так же выйти из приложения.
...
Теперь давайте рассмотрим исходный код:
- GameBase и Game - это классы, который содержит состояние игры, объекты представляющие реальных игроков, а так же реализует все игровые правила.
- PlayerBase и Player - это класс, который содержит состояние игрока, персонажа и камеру.
