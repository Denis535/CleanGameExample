...
- Основные модули находятся в папке Project.Assemblies.
...
- DebugScreen - это класс, отвечающий за вывод отладочной информации на экран.
- ProjectToolbar - это класс, содержащий разные полезные команды, которые упрощают работу с проектом. 
- ProjectWindow2 - это класс, модифицирующий окно проекта, что делает его более удобным.

# 5. Модуль Project.UI
В этом видео мы рассмотрим модуль Project.UI.
Данный модуль отвечает за пользовательский интерфейс нашего приложения.
Наш пользовательский интерфейс состоит из нескольких неймспейсов:
- Project.UI - содержит тему, экран и роутер.
- Project.UI.MainScreen - содержит виджеты главного экрана.
- Project.UI.GameScreen - содержит виджеты игрового экрана.
- Project.UI.Common - содержит общие виджеты. 
А так же, обратите внимание, что модуль разделен на две части:
- Project.UI - содержит аудио тему, графический экран, роутер и виджеты.
- Project.UI.Internal - содержит вьюшки.
Благодаря такому подходу, вьюшки не имеют зависимостей на другие модули.

Теперь давайте рассмотрим исходный код:
- UITheme - это класс, отвечающий за аудио тему.
  Тема использует паттерн состояние, что позволяет вам легко переключаться между разными плейлистами.
- PlayList, MainPlayList и GamePlayList - это состояния темы, отвечающие за проигрывание музыкальных плейлистов.

- UIScreen - это класс отвечающий за графический пользовательский интерфейс.
  Экран использует state-driven архитектуру, что позволяет вам легко создавать пользовательский интерфейс из множества разных виджетов и легко переключаться между ними.
  Я рассказывал об этой архитектуре в одном из прошлых видео.
- RootWidget - это корневой виджет, отвечающий за добавление других виджетов в дерево визуальных элементов.

- UIRouter - это класс, предоставляющий методы для управления состоянием приложения.
  Роутер может загружать, перезагружать и выгружать игру, а так же выходить из приложения.

- MainWidget - это экран главного меню.
...
Данный модуль отвечает за все сущности и сервисы необходимые для работы самого приложения.

Теперь давайте рассмотрим исходный код:
- Application - это класс, содержащий все сущности и сервисы.
...
Наша предметная область состоит из нескольких частей:
- Project.Entities - содержит базовые сущности, такие как игра, игрок, камера или игровой дисплей (HUD).
- Project.Entities.Actors - содержит действующие сущности, которые могут действовать под управлением игрока или самостоятельно.
