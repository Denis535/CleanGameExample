...
- Первая идея — это Чистая Архитектура.
  Это идея, что проект состоит из первостепенных и второстепенных модулей.
  То есть, главный модуль содержит все самое важное.
  А второстепенный модуль может содержать весь мусор.
...
- Вторая идея — это Domain-Driven Дизайн.
  Это идея, что проект состоит из предметной области и технических модулей.
  ...
  А остальные модули являются техническими и нужны только для обеспечения функционирования модулей предметной области. 
  В итоге получается, что предметная область полностью очищена от технических деталий.
  А ее разработка никак не зависит от других модулей.
...
- Четвертая идея — это State-Driven UI.
  ...
  Причем дерево состояний является первичным и абсолютно независимым от дерева визуальных элементов.
  Мы можем добавлять новые ноды в дерево состояний или удалять ноды и даже целые ветки.
  А каждое состояние, в свою очередь, может иметь (или не иметь) свое визуальное представление.
  Таким образом мы можем элегантно делать сложные пользовательские интерфейсы, состоящие из разных состояний и их комбинаций.
