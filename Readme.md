...
- Первая идея — это Чистая Архитектура.
  ...
  То есть допустим, главный модуль содержит все самое важное.
  ...
  В итоге получается, что все самое важное в проекте очищено от всего мусора.
...
- Вторая идея — это Domain-Driven Дизайн.
  ...
  Предметная область это множество сущностей, выполняющие бизнес задачи приложения и отражающие объекты реального или придуманного мира.
  То есть модули предметной области выполняют все бизнес задачи.
  А остальные модули являются техническими и нужны только для обеспечения функционирования предметной области.
  В итоге получается, что предметная область полностью очищена от технических деталий.
  А разработка предметной области никак не зависит от других модулей.
...
- Четвертая идея — это State-Driven UI.
  ...
  Мы можем добавлять новые ноды в дерево состояний или удалять ноды и даже целые ветки из дерева состояний.
