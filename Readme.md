...
**Проблема**
При разработке достаточно сложных проектов, проекты декомпозируется на множество разных модулей, неймспейсов и типов.
В идеале каждая единица проекта должна иметь единую ответственность и минимальное количество зависимостей.
...
- Первая идея — это Чистая Архитектура.
  Это идея, что проект состоит из набора конкретных модулей.
  А каждый модуль, в свою очередь, должен иметь конкретную ответственность и конкретные зависимости.
  Например, проект может состоять из точки входа, презентации, приложения, предметной области и модуля инфраструктуры.
...
- Вторая идея — это Domain-Driven Дизайн.
  ...
  В итоге получается, что предметная область полностью выделена в отдельные модули.
  А ее разработка становится более простой и удобной.

- Третья идея - инверсия зависимостей.
  Это идея, что модули предметной област не должны иметь зависимостей на технические модули.
  Вместо этого технические модули могут имеют зависимости на модули предметной области.
  Таким образом предметная область может предоставлять некоторые абстракции, а инфраструктура может реализовывать эти абстракции.
  Затем предметная область может использовать инфраструктуру через свои собственные абстракции.
  В итоге получается, что предметная область полностью очищена от технических деталей.
  А ее разработка становится полностью независимой от всего остального.

- Четвертая идея — это View-Driven UI.
...
- Пятая идея — это State-Driven UI.
...
- Первая идея — это декомпозиция проекта.
  ...
  Нужно понимать, что проект можно декомпозировать по назначению кода или по уровню абстракции кода или по каким-то другим критериям.
