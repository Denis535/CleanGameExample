...
- Первая идея — это декомпозиция проекта.
  ...
  В теории это просто, но на практике часто не понятно по каким критериям лучше декомпозировать проект.
  Обычно проект декомпозируется так, чтобы каждая единица проекта имела одну ответственность.
...
- Вторая идея — это Чистая Архитектура.
  Это идея, что проект декомпозируется на модули с конкретными ответственностями и зависимостями.
  То есть проект должен состоять из модулей точки входа, презентации, приложения и предметной области.
  А все зависимости направлены от первого модуля к последнему.
  Так же проект может иметь дополнительные модули инфраструктуры.
  В итоге получается, что проект состоит из простых модулей, каждый из которых имеет всем понятное назначение и минимальное количество зависимостей.
...
- Третья идея — это Domain-Driven Дизайн.
  ...
  А разработка такой предметной области становится более удобной.
...
- Четвертая идея - инверсия зависимостей.
  ...
  Но только при условии, что инфраструктура имеет реализации этих абстракций.
  В итоге предметная область полностью очищена от всех технических деталей.
  А разработка такой предметной области становится полностью независимой от всего остального.
