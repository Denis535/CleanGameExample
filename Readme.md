...
- Вторая идея — это разделение проекта и контента.
  ...
  Но заметьте, что некоторый контент все равно лучше хранить отдельно.
  Например, в папке Plugins я храню библиотеки, которые не имеют прямого отношения к нашему проекту.
...
- Первая идея — это Чистая Архитектура.
  Это идея, что проект состоит из первостепенных и второстепенных модулей, с конкретными ответственностями и зависимостями.
...
- Вторая идея — это Domain-Driven Дизайн.
  Это идея, что сердцем проекта являются сущности, выполняющие бизнес задачи приложения и отражающие объекты реального или придуманного мира.
  То есть модуль предметной области является самым главным в проекте.
  А остальные модули являются техническими и нужны лишь для обеспечения функционирования предметной области.
  В итоге получается, что модуль предметной области выполняет только лишь свои бизнес задачи и так же становится максимально простым и чистым.
  А другие модули никак не должны влиять на разработку предметной области.
...
- Четвертая идея — это State-Driven UI.
  ...
  Мы можем добавлять новые ноды в дерево состояний, и так же мы можем удалять ноды и даже целые ветки из дерева состояний.
  В итоге мы можем элегантно делать сложные пользовательские интерфейсы, состоящие из разных состояний и их комбинаций.
