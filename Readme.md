...
- Application - это сущность приложения.
  Она содержит другие сущности и сервисы.
- Storage, ProfileSettings, VideoSettings, AudioSettings - это объекты, содержащие разные значения.
  Эти значения могут быть загруженны из хранилища или полученны из аргументов окружения.
...
- EntityBase - это объект представляющий высокоуровневую единицу предметной области.
  ...
  Например: актер, вещь, транспортное средство, объект окружения или любые другие мелкие объекты.
...
- IDependencyContainer - это локатор служб, который предоставляем вам необходимые зависимости.
- UIRootWidgetBase - это корневой виджет.
  Он размещает другие виджеты на экране.
  А так же он обрабатывает события навигации и перенаправляет их в соответствующие кнопки.

Теперь давайте рассмотрим модуль Internal.
