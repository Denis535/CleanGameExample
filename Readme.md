...
- ProjectToolbar - это класс инструментов, содержащий набор разных полезные комманд, которые вы можете использовать на панели инструментов.
...
  Этот класс использует паттерн состояние, что позволяет вам легко переключаться между разными плейлистами.
...
- UIScreen - это класс графического пользовательского интерфейса нашего приложения.
  Этот класс использует state-driven архитектуру, что позволяет вам создавать пользовательский интерфейс из множества разных виджетов и легко переключаться между ними.
...
- RootWidget - это класс корневого виджета.
  Этот класс добавляет другие виджеты в дерево визуальных элементов.

- UIRouter - это класс менеджера нашего приложения.
  Этот класс предоставляет вам методы для управления приложением, такие как загрузка, перезагрузка и выгрузка игры, а так же выход из приложения.
...
- GameBase и Game - это сущность игры.
  Эта сущность содержит информацию, состояния, правила и игроков.
- PlayerBase и Player - это сущность представляющая реального игрока.
  Эта сущность содержит информацию, состояния и другие сущности принадлежащие игроку.
...
- PlayerCharacter - это сущность персонажа игрока, которая обрабатывает пользовательский ввод.
- EnemyCharacter - это сущность вражеского персонажа, которая имеет простейший искусственный интеллект.
...
Теперь давайте рассмотрим исходный код.
- Lock - это полезный класс, запрещающий одновременный многочисленный доступ в одну область кода.
...
- Socket - это контейнер для каких-либо объектов.
- VisualElementExtensions - это набор расширений для VisualElement.
- VisualElementFactory - это набор методов для создания разных визуальных элементов.
- IDamageable - это интерфейс для нанесения разных уронов любым объектам.
- ActorBase - это базовый класс для всех актеров, действующих под управлением игрока или искусственного интеллекта.
- MovableBody - это класс реализующий простейшую физику тела персонажа.
