...
- Вторая идея — это разделение проекта и контента.
  ...
  Эти папки привязаны к соответствующим модулям и адресуемым группам (addressables groups).
  А названия этих папок отражают названия соответствующих модулей и групп.  
  При необходимости, вы даже можем привязать несколько разных папок к одному модулю или группе.
  Это позволяет уменьшить вложенность папок.
  Конечно же некоторый контент все же лучше хранить где-то отдельно.
